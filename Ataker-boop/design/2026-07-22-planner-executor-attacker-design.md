...
- **НЕ добавляем вторую LLM** для Executor'а — оставляем существующие 17 мутаций.
...
- **ТОЛЬКО** `verdict` (GREEN/YELLOW/RED). Без layer — настоящий black-box.
- **НЕ** передаём ответ жертвы (контент ответа Крепости).
- **layer** пишется в Vault/planner_log для аналитики оператором, но НЕ попадает в FeedbackEntry.
...
> он уже знает verdict, bypassed из SecurityContext.
>
...
3. **Планировщик не видит ответ жертвы.** Только verdict + bypassed. Без layer — настоящий black-box.
...
> - `ATTACK_TEMPLATES` (15 категорий, 70 шаблонов) в generator.py
> - `MutationEngine` (17 мутаций: base64, homoglyph, zero_width, ...)
...
class PlannedAttack:
    ...
    mutations: List[str]                # какие мутации применить (из 17 существующих)
...
- [ ] Должен ли Творец видеть конкретные шаблоны из ATTACK_TEMPLATES (с текстом),
      ...
      но длиннее промпт)
- [ ] Список 17 мутаций передаём Творцу в системный промпт?
...
@dataclass
class FeedbackEntry:
    """Результат одной атаки для Творца. Без layer — настоящий black-box."""
...
- ❌ layer (Layer1-Regex / Layer2-Guard / ...) — пишется в Vault/planner_log, НЕ Творцу
...
class PlannerLogEntry:
    ...
    hypothesis: str                   # план на следующие итерации
    results_summary: str              # "5/20 bypassed, 3 YELLOW, 12 RED"
...
class AdversarialReport:
    ...
    by_layer: Dict[str, int]          # какие слои слабее (для оператора, НЕ в FeedbackEntry)
